...
namespace UMA
{
    public class UMARandomAvatar : MonoBehaviour
    {
        ...
        public string MeshToString(Mesh mesh, Material[] materials)
        {
            Mesh m = mesh;
        ...
        {
            int count = 0;
            foreach (Transform child in ParentObject.transform)//for (int i = 0; i <Selection.gameObjects.Length; i++)
            {
                var selectedTransform = child;
                ...
                if (avatar != null)
                {
                    var path = "Assets/saved/obj/" + avatar.name + ".obj";// EditorUtility.SaveFilePanel("Save obj static mesh", "Assets", avatar.name + ".obj", "obj");
                    Debug.Log(path);
